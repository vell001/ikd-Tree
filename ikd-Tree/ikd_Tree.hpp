...
*/
    ...
    template<typename PointType>
    inline KD_TREE<PointType>::KD_TREE(float delete_param, float balance_param, float box_length) {
    ...
    template<typename PointType>
    inline KD_TREE<PointType>::~KD_TREE() {
        stop_thread();
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Set_delete_criterion_param(float delete_param) {
        delete_criterion_param = delete_param;
    }

    template<typename PointType>
    inline void KD_TREE<PointType>::Set_balance_criterion_param(float balance_param) {
        balance_criterion_param = balance_param;
    }

    template<typename PointType>
    inline void KD_TREE<PointType>::set_downsample_param(float downsample_param) {
        downsample_size = downsample_param;
    }

    template<typename PointType>
    inline void KD_TREE<PointType>::InitializeKDTree(float delete_param, float balance_param, float box_length) {
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::InitTreeNode(KD_TREE_NODE *root) {
        root->point.x = 0.0f;
        root->point.y = 0.0f;
        root->point.z = 0.0f;
    ...
    template<typename PointType>
    inline int KD_TREE<PointType>::size() {
        int s = 0;
    ...
    template<typename PointType>
    inline BoxPointType KD_TREE<PointType>::tree_range() {
        BoxPointType range;
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
            if (Root_Node != nullptr) {
    ...
    template<typename PointType>
    inline int KD_TREE<PointType>::validnum() {
        int s = 0;
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
            if (Root_Node != nullptr)
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::root_alpha(float &alpha_bal, float &alpha_del) {
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::start_thread() {
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::stop_thread() {
    ...
    template<typename PointType>
    inline void *KD_TREE<PointType>::multi_thread_ptr(void *arg) {
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::multi_thread_rebuild() {
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::run_operation(KD_TREE_NODE **root, Operation_Logger_Type operation) {
        switch (operation.op) {
            case ADD_POINT:
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Build(PointVector point_cloud) {
        if (Root_Node != nullptr) {
            delete_tree_nodes(&Root_Node);
        }
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Nearest_Search(PointType point, int k_nearest, PointVector &Nearest_Points,
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Box_Search(const BoxPointType &Box_of_Point, PointVector &Storage) {
        Storage.clear();
        Search_by_range(Root_Node, Box_of_Point, Storage);
    }

    template<typename PointType>
    inline void KD_TREE<PointType>::Radius_Search(PointType point, const float radius, PointVector &Storage) {
        Storage.clear();
        Search_by_radius(Root_Node, point, radius, Storage);
    }

    template<typename PointType>
    inline int KD_TREE<PointType>::Add_Points(PointVector &PointToAdd, bool downsample_on) {
        ...
        for (int i = 0; i < PointToAdd.size(); i++) {
            if (downsample_switch) {
                ...
                mid_point.x =
                        Box_of_Point.vertex_min[0] + (Box_of_Point.vertex_max[0] - Box_of_Point.vertex_min[0]) / 2.0;
                mid_point.y =
                        Box_of_Point.vertex_min[1] + (Box_of_Point.vertex_max[1] - Box_of_Point.vertex_min[1]) / 2.0;
                mid_point.z =
                        Box_of_Point.vertex_min[2] + (Box_of_Point.vertex_max[2] - Box_of_Point.vertex_min[2]) / 2.0;
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Add_Point_Boxes(vector<BoxPointType> &BoxPoints) {
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Delete_Points(PointVector &PointToDel) {
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::acquire_removed_points(PointVector &removed_points) {
        pthread_mutex_lock(&points_deleted_rebuild_mutex_lock);
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::BuildTree(KD_TREE_NODE **root, int l, int r, PointVector &Storage) {
        if (l > r) return;
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Rebuild(KD_TREE_NODE **root) {
    ...
    template<typename PointType>
    inline int KD_TREE<PointType>::Delete_by_range(KD_TREE_NODE **root, BoxPointType boxpoint, bool allow_rebuild,
                                                   bool is_downsample) {
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Delete_by_point(KD_TREE_NODE **root, PointType point, bool allow_rebuild) {
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Add_by_range(KD_TREE_NODE **root, BoxPointType boxpoint, bool allow_rebuild) {
    ...
    template<typename PointType>
    inline void
    KD_TREE<PointType>::Add_by_point(KD_TREE_NODE **root, PointType point, bool allow_rebuild, int father_axis) {
        if (*root == nullptr) {
            *root = new KD_TREE_NODE;
            InitTreeNode(*root);
            (*root)->point = point;
    ...
    template<typename PointType>
    inline void
    KD_TREE<PointType>::Search(KD_TREE_NODE *root, int k_nearest, PointType point, MANUAL_HEAP &q, double max_dist) {
        if (root == nullptr || root->tree_deleted) return;
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Search_by_range(KD_TREE_NODE *root, BoxPointType boxpoint, PointVector &Storage) {
        if (root == nullptr) return;
        Push_Down(root);
    ...
    template<typename PointType>
    inline void
    KD_TREE<PointType>::Search_by_radius(KD_TREE_NODE *root, PointType point, float radius, PointVector &Storage) {
        if (root == nullptr)
            return;
        Push_Down(root);
    ...
    template<typename PointType>
    inline bool KD_TREE<PointType>::Criterion_Check(KD_TREE_NODE *root) {
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Push_Down(KD_TREE_NODE *root) {
        if (root == nullptr) return;
        Operation_Logger_Type operation;
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::Update(KD_TREE_NODE *root) {
        ...
        if (left_son_ptr != nullptr && right_son_ptr != nullptr) {
            ...
            root->tree_downsample_deleted =
                    left_son_ptr->tree_downsample_deleted & right_son_ptr->tree_downsample_deleted &
    ...
    template<typename PointType>
    inline void
    KD_TREE<PointType>::flatten(KD_TREE_NODE *root, PointVector &Storage, delete_point_storage_set storage_type) {
        if (root == nullptr) return;
        Push_Down(root);
        if (!root->point_deleted) {
            Storage.push_back(root->point);
    ...
    template<typename PointType>
    inline void KD_TREE<PointType>::delete_tree_nodes(KD_TREE_NODE **root) {
        if (*root == nullptr) return;
        Push_Down(*root);
    ...
    template<typename PointType>
    inline bool KD_TREE<PointType>::same_point(PointType a, PointType b) {
        return (fabs(a.x - b.x) < EPSS && fabs(a.y - b.y) < EPSS && fabs(a.z - b.z) < EPSS);
    }

    template<typename PointType>
    inline float KD_TREE<PointType>::calc_dist(PointType a, PointType b) {
        float dist = 0.0f;
    ...
    template<typename PointType>
    inline float KD_TREE<PointType>::calc_box_dist(KD_TREE_NODE *node, PointType point) {
...
// manual queue
    ...
    template<typename T>
    inline void MANUAL_Q<T>::push(T op) {
        q[tail] = op;
        counter++;
